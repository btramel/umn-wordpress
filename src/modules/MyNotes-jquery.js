...
class MyNotes {
    ...
    createNote() {
        ...
        $.ajax({
            beforeSend: (xhr) => {
                xhr.setRequestHeader('X-WP-Nonce', hogwartsData.nonce);
            },
            url: hogwartsData.root_url + '/wp-json/wp/v2/note/',
            type: 'POST', 
            data: ourNewPost,
            success: (response) => {
                $(".new-note-title, .new-note-body").val('');
                // template literal below is copy-pasted from page-my-notes.php, but the PHP is replaced with JS expressions
                $(`
    ...
    updateNote(e) {
        ...
        $.ajax({
            ...
            url: hogwartsData.root_url + '/wp-json/wp/v2/note/' + thisNote.data('id'),
            type: 'POST', 
            data: ourUpdatedPost,
            success: (response) => {
